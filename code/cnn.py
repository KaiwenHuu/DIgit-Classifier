...
class CNN():
    ...
    def conv(self, image, label, params, conv_s, pool_f, pool_s):
        ...
        probs = softmax(out)  # predict class probabilities with softmax
        ...
        dconv2 = maxpoolBackward(dpool, conv2, pool_f, pool_s)  # backpropagate through pooling layer
        dconv2[conv2 <= 0] = 0  # backpropagate through ReLU activation

        dconv1, df2, db2 = convolutionBackward(dconv2, conv1, f2, conv_s)  # backpropagate through second convolutional layer
        dconv1[conv1 <= 0] = 0  # backpropagate through ReLU activation

        dimage, df1, db1 = convolutionBackward(dconv1, image, f1, conv_s)  # backpropagate through first convolutional layer
